...
Settings::Settings() {
    frame = { (Page::width - FRAME_WIDTH) / 2.0f, (Page::height - FRAME_HEIGTH) / 2.0f + 2 * INDENT_HEADER,
             FRAME_WIDTH, FRAME_HEIGTH };
    volumeSliderBar = { frame.x + SHIFT_DESC + (SHIFT_DESC - VOLUME_BAR_WIDTH) / 2.0f, frame.y + INDENT_FRAME + 5,
...
void Settings::show() const
{
    ...
    for (size_t i = 0; i < text.size(); ++i) {
        ...
        DrawTextEx(font, text[i].c_str(), pos, FONT_SIZE, 5, BLACK);
    }
    ...
    std::transform(description.begin(), description.end(), bounds.begin(), [&](const std::string &str)
                   { return MeasureTextEx(font, str.c_str(), FONT_SIZE / 1.5, 5); });
    for (size_t i = 0; i < description.size(); ++i) {
        ...
        DrawTextEx(font, description[i].c_str(), pos, FONT_SIZE / 1.5, 5, BLACK);
    }
}

Pages Settings::update() {
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
        if (CheckCollisionPointCircle(GetMousePosition(), {BACK_BUTTON_X, BACK_BUTTON_Y},
            BACK_BUTTON_RADIUS)) colorBack = colorPressed;
        if (CheckCollisionPointRec(GetMousePosition(), volumeSliderBar)) {
            volumeSlider.x = GetMouseX() - volumeSlider.width / 2;
            if (volumeSlider.x < volumeSliderBar.x) volumeSlider.x = volumeSliderBar.x;
            if (volumeSlider.x > volumeSliderBar.x + volumeSliderBar.width - volumeSlider.width) volumeSlider.x = volumeSliderBar.x + volumeSliderBar.width - volumeSlider.width;
...
void Settings::setPrevPage(Pages page) {
    prevPage = page;
}
