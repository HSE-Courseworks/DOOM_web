...
void Pause::show() const {
    ...
    Vector2 boundsResume = MeasureTextEx(font, "Resume", FONT_SIZE, 5);
    ...
    DrawTextEx(font, "Resume", posPlay, FONT_SIZE, 5, BLACK);
...
Pages Pause::update() {
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
        if (CheckCollisionPointRec(GetMousePosition(), resumeButton)) colorResume = colorPressed;
...
void Pause::setPrevPage(Pages page) {
    prevPage = page;
}