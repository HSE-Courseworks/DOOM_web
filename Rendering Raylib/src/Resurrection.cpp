...
void Resurrection::show() const {
    ...
    if (GetTime() - whenDied < TIME_RESURRECTION) {
        Vector2 bounds = MeasureTextEx(font, "Restart after ...", FONT_SIZE, 5);
        DrawTextEx(font, "Restart after ...", {(Page::width - bounds.x) / 2.0f, (Page::height - bounds.y) / 2.0f},
               FONT_SIZE, 5, WHITE);
        ...
        (Page::height - boundsText.y) / 2.0f + FONT_SIZE + INDENT_HEADER}, FONT_SIZE * 2, 5, WHITE);
    }
    else {
        const char* text = "... Press space to restart ...";
        Color color = {255, 255, 255, alpha};
        Vector2 boundsText = MeasureTextEx(font, text, FONT_SIZE, 5);
        DrawTextEx(font, text, {(Page::width - boundsText.x) / 2.0f, 
        (Page::height - boundsText.y) / 2.0f}, FONT_SIZE, 5, color);
    }
}
...
void Resurrection::setTimeDied(const double time) {
    whenDied = time;
}