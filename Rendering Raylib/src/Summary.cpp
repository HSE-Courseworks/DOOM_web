...
void Summary::show() const {
    ...
    Vector2 boundsText = MeasureTextEx(font, "Winner: ", FONT_SIZE, 5);
    Vector2 boundsNick = MeasureTextEx(font, winner.c_str(), FONT_SIZE * 1.5f, 5);

    DrawTextEx(font, "Winner: ", {(Page::width - boundsText.x - boundsNick.x) / 2.0f, 
            position.y + TABLE_SIZE_Y + INDENT_FRAME}, FONT_SIZE, 5, WHITE);
    DrawTextEx(font, winner.c_str(), {(Page::width - boundsText.x - boundsNick.x) / 2.0f + boundsText.x, 
            position.y + TABLE_SIZE_Y + INDENT_FRAME - 0.25f * FONT_SIZE}, FONT_SIZE * 1.5f, 5, RED);

    if (GetTime() - whenEnd >= TIME_EXIT) {
        const char* text = "... Press space to menu ...";
        Color color = {255, 255, 255, alpha};
        Vector2 boundsText = MeasureTextEx(font, text, FONT_SIZE / 2, 5);
        DrawTextEx(font, text, {(Page::width - boundsText.x) / 2.0f, 
        position.y + TABLE_SIZE_Y + INDENT_FRAME + INDENT_ROW + FONT_SIZE}, FONT_SIZE / 2, 5, color);
    }
}
...
void Summary::setTimeEnd(double time) {
    whenEnd = time;
}

void Summary::setFinalScoreTable(const ScoreTable& scoreTable) {
