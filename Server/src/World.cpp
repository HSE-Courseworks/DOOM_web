...
void World::pushEvent(PlayerEvent ev, int id) {
    qEvents.push(std::make_tuple(ev, id));
}
...
sf::Packet& operator<<(sf::Packet& pack, World& world) {
    ...
    for (auto& pl : world.players) {
        pack << true;
        pack << pl.first << pl.second.first << *pl.second.second;
    }
    pack << false;
    //pack << world.fps;
    //for (auto& sl : world.slots) {
    //    pack << std::get<0>(sl.first).x << std::get<0>(sl.first).y << std::get<1>(sl.first) /*<< std::get<2>(sl.first)*/ << sl.second;
    //}
    pack << world.timeEnd;
    return pack;
}